"""使用随机投影方法进行降维
code heavily based on anomalib and sklearn
"""
...
class BaseRandomProjection:
    ...
    def fit(self, embedding: Tensor) -> "BaseRandomProjection":
        ...
        self.random_matrix = self._make_random_matrix(self.n_components_, n_features=n_features)#.to(device)
    ...
    def transform(self, embedding: Tensor) -> Tensor:
        ...
        if self.random_matrix is None:
            raise NotFittedError("`fit()` has not been called on RandomProjection yet.")
...
class SparseRandomProjection(BaseRandomProjection):
    ...
    def _make_random_matrix(self, n_components:int, n_features: int):
        ...
        if density == 1:
            ...
            components = paddle.zeros((n_components, n_features), dtype=paddle.float64)
            for i in range(n_components):
                ...
                data = paddle.to_tensor(np.random.binomial(1, 0.5, c_idx.size)) * 2 - 1
